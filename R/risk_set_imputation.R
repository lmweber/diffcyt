...
risk_set_cov_adjusted <- function(data, censored_variable, censoring_indicator,
  ...
  if (wc < 0 | wc > 1){
    warning("weight 'wc' in 'risk_set_cov_adjusted' not between 0 and 1, setting
            to 0.5")
    wc <- 0.5
  }
  if (nn < 1){
    warning("number of neighbors to use 'nn' in 'risk_set_cov_adjusted' too low,
            setting to 10")
    nn <- 10
  }
  n <- dim(data)[1]
  ...
  if (any(cox_coef_c == 0) | any(cox_coef_e == 0)){ # if coefficients are zero
