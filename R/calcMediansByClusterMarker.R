...
#' 
#' 
#' @return \code{d_medians_by_cluster_marker}: \code{\link{SummarizedExperiment}} object,
...
calcMediansByClusterMarker <- function(d_se) {
  ...
  d_all %>% 
    group_by(cluster_id, marker_id) %>% 
    summarize(median = median(value)) -> 
    medians
  
  medians <- acast(medians, cluster_id ~ marker_id, value.var = "median", fill = NA)
  
  # fill in any missing clusters
  if (nrow(medians) < nlevels(rowData(d_se)$cluster_id)) {
    ix_missing <- which(!(levels(rowData(d_se)$cluster_id) %in% rownames(medians)))
  ...
  row_data <- data.frame(
    cluster_id = factor(rownames(medians), levels = levels(rowData(d_se)$cluster_id)), 
    stringsAsFactors = FALSE
  )
  ...
  d_medians_by_cluster_marker <- SummarizedExperiment(
    assays = list(medians_by_cluster_marker = medians), 
    rowData = row_data, 
    colData = col_data, 
    metadata = metadata
  )
