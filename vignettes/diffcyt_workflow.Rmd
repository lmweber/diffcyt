...
Calculate cluster cell counts and cluster medians (median marker expression for each cluster and sample). These objects are required to calculate the differential tests and to generate plots. See `?calcCounts` and `?calcMedians` for more details.
...
Calculate medians for each cluster-marker combination (i.e. median marker expression for each cluster, across all samples) and medians for each sample-marker combination (i.e. overall median marker expression for each sample, across all cells). These additional objects are required for plotting purposes. See ?calcMediansByClusterMarker` and `?calcMediansBySampleMarker` for more details.
