...
# note: selecting columns containing group IDs and patient IDs
...
(Note: For the example below, we use an earlier version of the heatmap plotting function, which is included within the `diffcyt` package, instead of using `CATALYST`. This is done to streamline installation and compilation of the `diffcyt` package and vignette, by reducing dependencies. However, alternative code to generate the heatmap using `CATALYST` is also shown; this version includes additional formatting, and will usually be preferred.)
